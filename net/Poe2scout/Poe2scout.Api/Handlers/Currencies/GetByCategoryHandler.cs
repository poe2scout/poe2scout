...
public static class GetByCategoryHandler
{
  ...
  public record GetByCategoryResponse(int CurrentPage, int Pages, int Total, List<GetByCategoryResponse.Item> Items)
  {
    public record Item(
      ...
      int? CurrentQuantity)
    {
      public record PriceLogEntry(float Price, DateTime Time, int Quantity);

      public static Item FromModel(Models.CurrencyItemExtended model) => new(
        ...
        model.PriceLogs
          .Select(priceLog => priceLog is null
            ? null
            : new PriceLogEntry((float)priceLog.Price, priceLog.Time, priceLog.Quantity))
          .ToList(),
        model.CurrentPrice is null ? null : (float)model.CurrentPrice,
