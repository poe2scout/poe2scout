...
async def sync_metadata_and_icon(want_currency: CurrencyItem, repo: ItemRepository, client: PoeTradeClient, BASE_URL: str, REALM: str, LEAGUE: str):
    ...
    if query_response.status_code != 200:
        raise Exception(f"Search request failed for {want_currency.text} with status code {query_response.status_code}")
    ...
    if fetch_response.status_code != 200:
        raise Exception(f"Fetch request failed for {want_currency.text} with status code {fetch_response.status_code}")
