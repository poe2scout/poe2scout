...
async def GetHistory(
    ...
    history = await item_repository.GetItemPriceHistory(
        itemId, leagueId, logCount, logFrequency, endTime
    )

    if referenceCurrency != "exalted":
        referenceCurrencyItem = await item_repository.GetCurrencyItem(referenceCurrency)
        referenceCurrencyHistory = await item_repository.GetItemPriceHistory(
            referenceCurrencyItem.itemId, leagueId, logCount, logFrequency, endTime
        )

        referenceCurrencyHistoryLookup = {
            log.time: log for log in referenceCurrencyHistory.price_history
        }
        ...
        for log in logs:
            ...
            if lastReferencePrice == 0:
                continue
            newLogEntry = PriceLogEntry(
                price=log.price / lastReferencePrice,
                time=log.time,
