...
from services.repositories.item_repository.CreateItemCategory import CreateItemCategoryModel
...
async def sync_items(categories: list[itemCategory]):
    ...
    all_types, all_base_items, all_items, all_categories, all_unique_items = await refresh_lists(repo)
    ...
    for category in categories:
        ...
        if not category_exists:
            logger.info(
                f"Creating new category: {category.label}, {category.id}")
        ...
        for item_entry in category.entries:
            ...
            if not type_exists:
                ...
                type_id = next(
                    t.id for t in all_types if t.value == item_entry.type)
            ...
            if is_unique(item_entry):
                # Create Item
                unique_exists = any(
                    u.name == item_entry.name for u in all_unique_items)
