...
async def GetAllItems(
    league: str, item_repository: ItemRepository = Depends(get_item_repository)
) -> list[UniqueItemResponse | CurrencyItemResponse]:
    ...
    for item_id, logs in price_logs.items():
        last_prices[item_id] = next(
            (log.price for log in logs if log and hasattr(log, "price")), 0
        )
