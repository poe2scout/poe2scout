...
from src.services.repositories.models import CurrencyItemExtended
...
async def GetCurrencyItems(
    category: str,
    econCache: EconomyCacheDep,
    ...
    items = await econCache.GetCurrencyPage(
        leagueInDb.id, category, search, referenceCurrency
    )
    itemCount = len(items)
