...
async def run(
    ...
    await asyncio.gather(
        run_currency_exchange_prices(config), 
        fetch_prices()
    )
...
async def fetch_currency_exchange_prices(
    ...
    try:
        for league in leagues:
            if await price_log_repository.get_prices_checked(current_epoch, league.league_id):
                logger.info(
            ...
            if len(chaos_pairs) == 0:
                ...
                item_price = await price_log_repository.get_item_price(
                    (await currency_item_repository.get_chaos_item()).item_id, 
                    league.league_id, 
                    current_epoch
                )
                if item_price == 0:
            ...
            if len(divine_prices) == 0:
                ...
                item_price = await price_log_repository.get_item_price(
                    (await currency_item_repository.get_divine_item()).item_id, 
                    league.league_id, 
                    current_epoch
                )
                if item_price == 0:
