...
async def run_once(client: Client) -> None:
    games = await game_repository.get_games()

    for game in games:
        logger.info("Fetching unique items from POE API...")
        response = client.get(
            f"https://www.pathofexile.com/api/{game.ggg_api_trade_identifier}/data/items"
        )
        ...
        response = client.get(
            f"https://www.pathofexile.com/api/{game.ggg_api_trade_identifier}/data/static"
        )
