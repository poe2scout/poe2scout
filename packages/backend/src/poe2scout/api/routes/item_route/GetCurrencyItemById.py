...
from poe2scout.api.routes.item_route.GetCurrencyItems import CurrencyItemExtended
from . import router
...
async def GetCurrencyItemById(
    apiId: str, league: str, repo: ItemRepository = Depends(get_item_repository)
    ...
    items.sort(
        key=lambda item: (lastPrice[item.itemId] if item.itemId in lastPrice else 0),
        reverse=True,
    )
    ...
    return GetCurrencyItemsResponse(item=item)
